    ...
    <div>
      ...
      {subject && (
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm mb-8">
          ...
          {subject.description && <p className="mt-4 text-gray-700">{subject.description}</p>}
      ...
      <div className="grid gap-4">
        {topics.map((topic) => (
          <div key={topic.id} className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
              ...
              <div className="flex flex-wrap gap-2">
                <Link href={`/subjects/${subjectId}/topics/${topic.id}`} className="rounded-md bg-slate-100 px-3 py-2 text-sm text-slate-900 hover:bg-slate-200">
                  View
                </Link>
                <Link href={`/subjects/${subjectId}/topics/${topic.id}/edit`} className="rounded-md bg-slate-100 px-3 py-2 text-sm text-slate-900 hover:bg-slate-200">
                  Edit
                </Link>
                <button
                  type="button"
