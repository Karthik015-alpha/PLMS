    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">Manage your subject list and track progress.</p>
        </div>
      ...
      <div className="grid gap-4">
        {subjects.map((subject) => (
          <div key={subject.id} className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
              ...
              <div className="flex flex-wrap gap-2">
                <Link href={`/subjects/${subject.id}`} className="rounded-md bg-slate-100 px-3 py-2 text-sm text-slate-900 hover:bg-slate-200">
                  View
                </Link>
                <Link href={`/subjects/${subject.id}/edit`} className="rounded-md bg-slate-100 px-3 py-2 text-sm text-slate-900 hover:bg-slate-200">
                  Edit
                </Link>
                <button
                  type="button"
